...
public final class SellService {
	...
	public void load() {
		...
		if (!file.exists()) {
			...
			save();
			plugin.getLogger().info("Created sell-prices.yml with " + prices.size() + " DonutSMP-scale prices.");
			return;
		}
		...
		if (fileVersion < PRICE_TABLE_VERSION) {
			prices.putAll(defaults);
			save();
			plugin.getLogger().info("Upgraded sell-prices.yml to DonutSMP-scale v" + PRICE_TABLE_VERSION
					+ " (" + prices.size() + " prices).");
			return;
		}
	...
	public void save() {
		...
		data.options().header(String.join("\n",
				"DonutSMP-scale fixed sell prices (base 1.0x) for every item/block.",
				"Confirmed anchors: oak log $300, sand $100, diamond $1,200, leather $10,000,",
				"bamboo block $500, dried kelp block $550, ancient debris $1,700,000.",
	...
	/**
	 * DonutSMP-scale base /sell prices (1.0×).
	 * Confirmed wiki anchors are hardcoded; everything else is extrapolated to that economy.
	 */
	static double computePrice(Material mat) {
		String n = mat.name();

		// === Confirmed DonutSMP wiki anchors ===
